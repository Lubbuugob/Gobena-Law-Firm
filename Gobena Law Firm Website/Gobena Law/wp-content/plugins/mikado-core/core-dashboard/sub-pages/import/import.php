...
class MikadoCoreImport {
	...
	public function import_action() {
			...
			if (isset($_POST) || !empty($_POST) || isset($_POST['options']['demo'])) {
					...
					switch ($_POST['options']['action']):
						...
						case 'rev-slider':
							if (mkd_core_is_revolution_slider_installed()) {
								...
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'mikado-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!MikadoCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									mkd_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'mikado-core'));
									exit;
								}
							}
                ...
                mkd_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'mikado-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $mikado_widget_id => $mikado_widget_data) {
				update_option('widget_' . $mikado_widget_id, $mikado_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'mikado-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$mikado_sidebars = get_option( "sidebars_widgets" );
		unset( $mikado_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$mikado_sidebars = array_merge( (array) $mikado_sidebars, (array) $data['sidebars'] );
			unset( $mikado_sidebars['wp_inactive_widgets'] );
			$mikado_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $mikado_sidebars );
			$mikado_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $mikado_sidebars );
			return true;
		} else {
			return false;
		}
		...
		if($options){
			if($current_options != $options) {
				...
				if ($result) {
					...
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'mikado-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'mikado-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'mikado-core'));
			}
		}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_mikado_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'mikado-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'mikado-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'mikado-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'mikado-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'mikado-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( MIKADO_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//
		...
		$mikado_import = new WP_Import();
		set_time_limit( 0 );

		$mikado_import->fetch_attachments = $attachments;
		$returned_value                  = $mikado_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'mikado-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'mikado-core' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'mikado-core'));
					exit;
				}
			}
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'mikado-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'mikado-core'));
		}
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
				...
				$html = mkd_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					mkd_core_ajax_status( 'success', '', $html);
				} else {
                    mkd_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'mikado-core' ), '');
				}
			}
		}
